...
export default class extends Vue {
    ...
    builtBoxSchedule = (classBox: string, index: number, subjectMatter: subjectMatter, room: string) =>
    ...
    insertSchedule(IndexstartHour: number, day: string): string {
        ...
        this.subjectMatters.forEach((subjectMatter, index) => {
            subjectMatter.schedules.forEach(schedule => {
                const hours: hourHandler = new hourHandler(this.hours.data[IndexstartHour], this.hours.data[IndexstartHour + 1], schedule)
                ...
                if (this.compareDay(day, schedule.day) && hours.compareHour()) {
                    ...
                    inyect = this.builtBoxSchedule(classBox, index, subjectMatter, schedule.room)
                }
            })
        })
        return inyect
