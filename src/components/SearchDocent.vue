...
export default class extends Vue {
  ...
  async mounted() {
    try {
      this.inputSearch = document.querySelectorAll(".search_search input")[0];
  ...
  verifyTeacher(event: any = null) {
    const expReg = RegExp(this.inputSearch.value.toUpperCase());
    this.SearchTeachers = [];
    ...
    if (event !== null && event.key === "Enter")
      if (this.SearchTeachers.length === 1)
        this.fillInput(this.SearchTeachers[0]);
    this.search();
  }

  fillInput(name: string) {
    this.inputSearch.value = name.toUpperCase();
    this.SearchTeachers = [];
  }

  search() {
    this.$store.commit("stalking/mutationTeacher", this.inputSearch);
  }
}
</script>
